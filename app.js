...
async function isRoomCodeInDB(code){
  let result = await roomCollection.findOne({code: code});
...
async function addUserInDB(username, password){
  let result = await userCollection.insertOne({username: username, password: password});
  return result ? true : false;
}

async function updateRoomCodesInDB(code, access_token, refresh_token){
  let query = {code: code};
  let updated = {$set: {access_token: access_token, refresh_token: refresh_token}};
...
async function updateCurrTrack(code, newCurrTrack){
  let query = {code: code};
  let updated = { $set: { currentTrack: newCurrTrack } };
  let result = await roomCollection.updateOne(query, updated);
  return result;
}
...
async function updateRoomLock(code, lock){
  let query = {code: code};
  let updated = { $set: { queueLock: lock } };
  let result = await roomCollection.updateOne(query, updated);
  return result;
}
...
async function updateRoomQueue(code, newQueue){
  let query = {code: code};
  let updated = { $set: { queue: newQueue } };
  let result = await roomCollection.updateOne(query, updated);
  return result;
}
...
async function deleteRoom(code){
  let result = await roomCollection.deleteOne({code: code});
  return result;
}
      ...
      if (!error && response.statusCode === 200) {
          if (body){
            var createPlaylistOptions = {
              ...
              body: JSON.stringify({'name': 'POQ_' + generateRandomString(4)})
            };
...
app.post('/refreshtoken', function(req, res){
  ...
  refreshToken(refresh_token, function(new_access_token){
    updateUserCodesInDB(username, new_access_token, refresh_token).then(function(updateResult){
      if (updateResult){
        ...
        res.status(404);
        res.send({
          result: "Update failed, try again"
        });
      }
    });
  });
...
mongo.connectToServer( function( err, client ) {
  ...
  var deleteRoom2 = async function(code){
    await roomCollection.deleteOne({code: code}, (err, result) => {
      if (err){
        throw err;
      }
    });
  ...
  var deleteUser = async function(username){
    await userCollection.deleteOne({username: username}, (err, result) => {
      if (err){
        throw err;
      }
    });
